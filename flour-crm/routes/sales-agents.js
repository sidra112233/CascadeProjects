...
router.put('/:id',requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, email, commission_rate, is_active } = req.body;

    const [result] = await db.query(
      `UPDATE sales_agents SET name=?, email=?, commission_rate=?, is_active=? WHERE id=?`,
      [name, email, commission_rate, is_active ?? 1, id]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await db.query(`DELETE FROM sales_agents WHERE id=?`, [id]);
...
router.post('/onboard', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const rawPassword =
      password && String(password).trim().length >= 6 ? String(password) : 'ChangeMe123!';
    const passwordHash = await bcrypt.hash(rawPassword, 10);

    // keep permissions JSON in case you want custom agent perms
